...
import { Flame, Star, Award, RotateCcw, BookOpen, Atom, FileCode } from "lucide-react";
...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      {/* Profile Header Card */}
      <div className="card-duo p-8 bg-white border-2 border-slate-200 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 shadow-xs">
        <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
          ...
          <div className="space-y-1">
            ...
            <div className="text-[#1cb0f6] font-extrabold text-sm uppercase tracking-wider">
              Tingkat {levelProgress.level}: {levelProgress.title}
            </div>
            <p className="text-xs font-bold text-slate-400">
              Anggota Pembelajar Aktif CodeQuest
            </p>
          </div>
        </div>
        ...
        <button
          onClick={() => {
            if (confirm("Apakah kamu yakin ingin mengatur ulang seluruh kemajuan belajar?")) {
              resetProgress();
            }
          }}
          className="btn-3d btn-outline-3d px-4 py-2 text-xs text-red-500 border-red-200 hover:bg-red-50"
        >
          <RotateCcw className="w-3.5 h-3.5 mr-1 inline-block" /> Atur Ulang Kemajuan
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="card-duo p-5 bg-amber-50 border-2 border-amber-200 rounded-2xl text-center space-y-1">
          ...
          <span className="text-[10px] font-extrabold uppercase text-amber-600 block">Total Poin</span>
        ...
        <div className="card-duo p-5 bg-orange-50 border-2 border-orange-200 rounded-2xl text-center space-y-1">
          ...
          <span className="text-[10px] font-extrabold uppercase text-orange-600 block">Streak Harian</span>
          <span className="text-2xl font-black text-orange-600">{user.streak} Hari</span>
        </div>
      ...
      <div className="card-duo p-6 md:p-8 bg-white border-2 border-slate-200 rounded-3xl space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-sm font-extrabold text-[#4b4b4b]">
              <span className="flex items-center gap-2">
                <BookOpen className="w-4 h-4 text-[#1cb0f6]" /> TypeScript
              </span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-sm font-extrabold text-[#4b4b4b]">
              <span className="flex items-center gap-2">
                <Atom className="w-4 h-4 text-purple-600" /> TSX / React
              </span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-sm font-extrabold text-[#4b4b4b]">
              <span className="flex items-center gap-2">
                <FileCode className="w-4 h-4 text-[#58cc02]" /> Python
              </span>
